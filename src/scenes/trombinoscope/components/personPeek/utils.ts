...
export const displayName = (p: any | null | undefined) =>
  (p && (p as any).primaryAttributes
    ? ((p as any).primaryAttributes ?? [])
        .filter((x: any) => !!x?.value)
        .map((x: any) => x.value.trim())
        .join(" ")
        .trim()
    : "") || `#${(p as any)?.idPerson ?? "?"}`;
...
export function useAttributeMeta(source: Attribute[] | undefined) {
  return useMemo(() => {
    ...
    (source || []).forEach((a) => { if (a?.id != null) map.set(a.id, a); });

    const getAttrMeta    = (id: number) => map.get(id);
    const getAttrLabel   = (id: number) => map.get(id)?.name ?? `Attribut #${id}`;
    const getAttrOrder   = (id: number) => map.get(id)?.displayOrder ?? Number.MAX_SAFE_INTEGER;
    ...
    const isLongTextAttr = (id: number, value: string) => {
      ...
      const longByValue = (value?.length ?? 0) > 80 || /\n/.test(value ?? "");
...
export function mapLiteToExtra(list: any[] | undefined | null): PersonAttributeExtraDto[] {
  if (!list) return [];
  const out: PersonAttributeExtraDto[] = [];

  for (const it of list as any[]) {
    const idRaw    = it?.attributeId ?? it?.idAttribute ?? it?.attribute?.id ?? it?.id;
    const valueRaw = it?.value ?? it?.rawValue ?? it?.text ?? it?.code ?? it?.label;
    const orderRaw = it?.displayOrder ?? it?.attribute?.displayOrder;

    const attributeId = idRaw != null ? Number(idRaw) : undefined;
    const value = valueRaw != null ? String(valueRaw) : undefined;
    const displayOrder: number | null = orderRaw == null ? null : Number(orderRaw);
